...
import { CORPUS_FIXTURES } from "@/lib/programme/corpus-fixtures"
import {
...
export async function createWritingGuide(workspaceId: string, instructions?: string) {
  const { createWorkspaceDocument, generateWorkspaceDocumentDraft } = await import("@/lib/actions/document")
  const { t } = await getServerTranslator()
  const created = await createWorkspaceDocument(workspaceId, {
    title: t("workspace.programme.documentRoles.programme_handbook"),
    content: "<p></p>",
    classification: "internal",
  })
  if (created.error || !created.data) return { error: created.error || "Could not create a writing guide" }
  const bound = await bindProgrammeDocumentRole(workspaceId, created.data.id, "programme_handbook")
  if (bound.error) return bound
  const prompt = instructions?.trim()
  if (prompt) {
    const draft = await generateWorkspaceDocumentDraft(workspaceId, created.data.id, {
      instructions: prompt,
      citationMode: "standard",
    })
    if (draft.error) return { error: draft.error, data: bound.data }
  }
  return bound
}
