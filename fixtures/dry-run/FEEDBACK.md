...
- **Analysis purpose copy starts with “Saved reports…”** Easy to read as a finished run. Empty state should not say “Saved”. **Fixed in this batch.** An empty Analysis sheet says to run analysis; it does not start with “Saved reports”.
- **Ask / Guidance stay mounted and hide with CSS.** Hidden Help / Close / tabs remain in the accessibility tree off-screen. Prefer `inert` on the closed rail. **Fixed in this batch.** The closed rail is `inert`.
- **Publish toolbar control** is “Publish status: Not published”, not a button labelled exactly “Publish”. Icon-only toolbar is easy to miss. **Fixed in this batch.** The control is labelled Publish. Publish itself stays closed until required chapters are approved and the owner freezes.
...
- **Opinion:** the comment rail is tight once a thread + the prep card stack. A later pass could collapse addressed themes and keep only the active paragraph card expanded. **Fixed in this batch.** Open themes collapse to one line while a paragraph is active, and inactive threads hide reply and resolve controls.
...
- **After measures, the bar says Writing and skips Effects.** Generation already stores a direction (here, Positive) on each measure, and the coach treats any non-empty direction as the effects check being done. The strip should stay on Effects until someone has recorded directions on the Effects sheet. **Fixed in this batch.** A generated Positive no longer counts. Each measure has to be saved once on the Effects sheet. Programmes that already saved effects before this change need that save again.
- **Save effects does not look finished.** After a successful save the card is unchanged: same fields, same Save effects button. It should show a saved state. **Fixed in this batch.** A recorded measure shows Saved and the button stays disabled until the draft changes.
- **Next: Writing still says Continue and highlights the first chapter.** Continue does not name a chapter. The first outline row (Inleiding en wettelijk kader) is the one that looks active. For this casus the chapter to write is Wonen en samenleving, not the first heading. The bar should point at the chapter that can actually be drafted from the bound sources. **Fixed in this batch.** Continue names that chapter and opens it. Framing headings (Inleiding, Visie) are skipped when a later empty chapter exists.
- **A click does not leave that first chapter.** Clicking the page, the bar, or the empty part of Inleiding left it active, and clearing it snapped back to the first chapter. **Fixed in this batch:** a click that is not another chapter or an edit control clears the chapter and it stays clear. Clicking a different chapter still opens that chapter.
- **Regenerate hides the chapter until the draft returns.** Choosing Regenerate with strict citations on Wonen closed the menu onto the page, which cleared the open chapter. The body and the chapter-tools icon vanished until generation finished, then the text came back without the tools. **Fixed in this batch:** a menu action does not count as clicking away, and the draft stays on screen while it runs.
- **Citations were raw JSON in the chapter.** A quote marker such as `[citation:{"quote":…}]` sat in the prose as ordinary text, and bold inside a quote split the marker so part of the JSON stayed visible. **Fixed in this batch:** the chapter shows a short source link (document role and page, for example “Environmental vision, p.1”). The link opens the source. Hover shows the document title and the full quote. The stored marker is unchanged, so groundedness and export still read it.
- **Opening that source said “Document not found.”** Sample sources are stored as text on the document, with no uploaded file and no pages. The text viewer asked for a `url` column that does not exist, so the lookup failed before it could read the stored text. **Fixed in this batch:** the viewer reads `external_url`, and when there is no file it shows `documents.content`.
- **The Review sheet needs to be simplified.** It is one long mix: colleague notes, then every generated measure (Request review / Request changes / Approve, reviewer, comment), then Chapters at the bottom. Wonen is easy to miss among the measure cards. Request review on Wonen moved it to `[in_review]` and enabled Approve chapter. Approving it then removed the row from the list, with no remaining sign on that sheet that Wonen was approved. **Fixed in this batch.** Colleague notes and the measure list are collapsed. Chapters stay on the sheet, with a readable status, and an approved chapter stays visible as Chapter approved.
- **After Wonen is approved, the strip still says Review.** Review is complete only when every chapter is approved, including headings that were never drafted (Mobiliteit and the rest of the standard outline). Approving the chapter that was written does not move the strip to Export. **Fixed in this batch.** Only chapters that have a draft must be approved. Undrafted headings do not keep the strip on Review. The strip then offers Export.
- **Word export does not turn chapter citations into footnotes.** Opened in Google Docs (no Word installed). The Wonen prose still contains the raw `[citation:{"quote":…}]` text. Measure citations become a numbered **Bronnen** list at the end, not footnotes on the sentences. The on-screen short links are display-only. **Print / PDF** is the browser print dialog saved as `Flevoland Dry Run.pdf` (7 pages). It is the same text: raw citation JSON in Wonen, `[^1]` markers in the measures appendix, **Bronnen** at the end, empty chapters as the Dutch purpose lines, and a header that says `about:blank`. **Fixed in this batch.** Chapter and measure citations become footnotes: a Word footnote in DOCX, and a numbered note under the print page. The print window uses the programme address instead of `about:blank`.
- **A drafter cannot add a citation while editing.** New sentences stay uncited unless the whole chapter is regenerated. Agreed 22 Sep: from the cursor, pick a bound source and a passage from it. The chapter shows the same short link; the hover shows the quote. **Fixed in this batch.** Select text, choose Cite, pick a bound source and a sentence. The same short link appears; hover still shows the quote.
- **This pass stops before Publish.** Publish stays closed until every required chapter is approved and the document owner freezes the programme. That repeats the Review wall, including chapters that were never written. Do not continue this programme into Publish until the Review sheet and the export footnotes are fixed.
