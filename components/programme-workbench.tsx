...
import { deriveGuidancePipeline, pipelineInputFromWorkbench, PIPELINE_STAGES } from "@/lib/guidance/pipeline"
...
export function ProgrammeWorkbench({
  ...
  const [viewJob, setViewJob] = useState<GuidanceJob | null>(null)

  const placeJob = effectiveJob({ spaceJob, workspaceJob, pathname })
  const chromeJob = viewJob ?? placeJob
  ...
  return (
    ...
    <div className="mx-auto flex max-w-6xl flex-col gap-6 p-6">
        ...
        <div>
          <p className="text-sm text-muted-foreground">
            <Link href={`/spaces/${spaceId}`} className="hover:underline">
              {spaceName || t("workspace.programme.back")}
            </Link>
          </p>
          <h1 className="text-2xl font-semibold">{workspaceName}</h1>
          <p className="text-sm text-muted-foreground">{t("workspace.programme.kindEnvironmental")}</p>
        </div>
        <div className="flex flex-wrap items-center gap-2">
          {spaceJob === "administrator" && (
            <label className="flex items-center gap-2 text-xs text-muted-foreground">
              {t("guidance.header.switchJob")}
              <Select value={chromeJob} onValueChange={(value) => setViewJob(value as GuidanceJob)}>
      ...
      <Tabs value={activeSection} onValueChange={setSection} className="space-y-6">
        ...
        >
          {PROGRAMME_WORKBENCH_SECTIONS.map((section) => (
            <TabsTrigger
              key={section}
              value={section}
              className={primarySections.includes(section) ? "text-xs sm:text-sm" : "hidden"}
            >
            ...
            <DropdownMenu>
              <DropdownMenuTrigger asChild>
                <Button type="button" variant="ghost" size="sm" className="text-xs sm:text-sm">
                  {t("guidance.header.more")}
                </Button>
              </DropdownMenuTrigger>
              <DropdownMenuContent align="start">
                {moreSections.map((section) => (
                  <DropdownMenuItem key={section} onSelect={() => setSection(section)}>
                    {t(`workspace.programme.nav.${section}`)}
                  </DropdownMenuItem>
                ))}
                {showConfiguration && (
        ...
        <TabsContent value="overview" className="space-y-4">
          ...
          <ol className="space-y-1 text-sm">
            {PIPELINE_STAGES.map((stage) => (
              <li key={stage} className={pipeline.stages[stage] ? "text-muted-foreground" : ""}>
