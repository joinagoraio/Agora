...
export function SpaceAgentsPanel({ spaceId, tenantId, compact = false, initialAgents = [] }: Props) {
  const { t } = useI18n()
  ...
  const { viewMode, setViewMode } = useCollectionViewMode(agents.length, `space.${spaceId}.agents`)
  ...
  return (
    <div className={compact ? "space-y-4" : cn("flex min-h-0 flex-col overflow-hidden", open ? "flex-1" : "shrink-0")}>
      <div className={cn("flex shrink-0 flex-wrap items-center justify-between gap-3", (compact || open) && "mb-4")}>
        <div>
          <div className="flex items-center gap-1">
            {compact ? null : (
              <SectionOpenToggle open={open} onToggle={toggle} label={t("space.agents.sectionTitle")} />
            )}
      ...
      {compact || open ? agents.length === 0 ? (
        <Card className="border-dashed">
          <CardContent className="flex flex-col items-center justify-center gap-3 py-10 text-center">
            ...
            <div>
              <h4 className="text-base font-semibold text-foreground">{t("space.agents.emptyTitle")}</h4>
        ...
        <div className={compact ? "" : "scrollbar-on-hover min-h-0 flex-1 overflow-y-auto"}>
        ...
        <Card className="flex min-h-0 flex-1 flex-col overflow-hidden py-0 shadow">
      ...
      {compact ? (
        ...
        <Dialog open={modelsOpen} onOpenChange={setModelsOpen}>
          <DialogContent className="max-h-[90vh] overflow-y-auto sm:max-w-2xl">
            ...
            <SpaceLlmSettings spaceId={spaceId} />
          </DialogContent>
        </Dialog>
      )}
