...
-- UPDATED: Handles existing policies gracefully
...
DROP POLICY IF EXISTS "System can insert document pages" ON public.document_pages;
DROP POLICY IF EXISTS "System can update document pages" ON public.document_pages;
DROP POLICY IF EXISTS "System can delete document pages" ON public.document_pages;
...
BEGIN
  IF NOT EXISTS (
    ...
    WHERE tgname = 'update_document_pages_updated_at'
  ) THEN
